...
export default class LoginOnly {
  static async execute() {
    logMessage(null, null, "Starting Login Only Mode...", "info");
    ...
    logMessage(null, null, "Login Only Process Completed!", "success");
  }
  ...
  static async processBatch(
    ...
    try {
      for (let i = 0; i < accounts.length; i++) {
        ...
        await this.loginWithoutImport(browser, accountEmail, password, i);
      }
    } catch (error) {
      logMessage(
        null,
  ...
  static async loginWithoutImport(browser, email, password, accountIndex) {
    const page = await browser.newPage();
    try {
      if (accountIndex === 0) {
        ...
        await page.goto("https://accounts.google.com/AddSession", {
          waitUntil: "networkidle2",
        });
      }
      ...
      try {
        ...
        if (emailExists) {
          ...
          logMessage(
            null,
            null,
            `[${email}] Already logged in or selector mismatch.`,
            "info",
          );
        }
      } catch (e) {
      ...
      try {
        ...
        try {
          await page.waitForSelector(workspaceListSelector, { timeout: 10000 });
          await new Promise((r) => setTimeout(r, 3000));
        } catch (e) {}
        ...
        if (currentWorkspaces < 2) {
          ...
          logMessage(
            null,
            null,
            `[${email}] Need to import ${needed} more workspace(s).`,
            "warning",
          );
          ...
          for (let j = 0; j < needed; j++) {
            logMessage(
              null,
              null,
              `[${email}] Importing Workspace ${j + 1}/${needed}...`,
              "process",
            );
            try {
              ...
              try {
                ...
                logMessage(
                  null,
                  null,
                  `[${email}] Workspace import started/confirmed.`,
                  "success",
                );
                logMessage(
                  null,
                  null,
                  `[${email}] Waiting for workspace to be active... (10s)`,
                  "debug",
                );
              ...
              logMessage(
                null,
                null,
                `[${email}] Import failed: ${err.message}`,
                "error",
              );
            }
          }
          ...
          logMessage(
            null,
            null,
            `[${email}] Account has sufficient workspaces.`,
            "success",
          );
        }
      } catch (err) {
        logMessage(
          null,
          null,
          `[${email}] Failed to check/import workspaces: ${err.message}`,
          "error",
        );
      }
    } catch (error) {
